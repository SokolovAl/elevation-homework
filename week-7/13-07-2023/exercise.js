...
class Principal extends Person {
    ...
    expelStudent(student) {
        const index = this.students.findIndex(stud => stud.name === student.name);
        if (index !== -1) {
    ...
    transferStudent(student, otherPrincipal) {
        const index = this.students.findIndex(stud => stud.name === student.name);
        if (index !== -1) {
            const transferredStudent = this.students.splice(index, 1)[0];
            otherPrincipal.recruitStudent(transferredStudent);
        }
    }
}
