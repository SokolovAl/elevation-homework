...
let biggestCountry = "";
let biggestPopulation = 0; // The variable for storing the largest population should be initialized to zero, and not
                           // taken from countries[0].population

for (let c of countries) {
    if (c.population > biggestPopulation) {
        biggestPopulation = c.population; // Update the value of the largest population
...
for (let c of countries) {
    if (c.landmarks) { // Adding check for landmarks
        let landmarks = c.landmarks;


        for (let l of landmarks) { // Here we fix it with "of" instead of "in" for proper iteration over landmarks
